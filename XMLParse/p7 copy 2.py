...
pja 5/30/2020 - final edits for ontology
pja 5/28/2020 - converted to ontology database
...
OperationWrite(name1,value1,name2,value2):
OPERATIONAL "first" first KRID kris ((first,KRID)) => kri
...
#
def xInit(fn,trace):
    global s,pu,fsz,ftsw,DeNftsw,sequenceMode,chooseMode,masterPKI,masterPU,seq,parentPosition,previousPosition,p2pix,gtrace,og1
    gtrace = trace
    p2pix = 0 # phase 2 index
    previousPosition = 0
    og1 = OntologyClass.Ontology(fn+".Onto",trace)
    seq = 0
    masterPKI=0
    masterPU = 0
    sequenceMode = 0
...
def ctl1():
    ...
    global s,m,r0,r1,rx,pu,l2x,fsz,ftsw,parent,attr,previousPosition,DeN,l1x,og1
    print('\n\n ctl1 begin ')
    attr = {} #blank 
    r1 = ''
    r0 = ''
    ...
    if (fsz <= s.fioxGet()):
       ctl = -100
        ...
        #previous,pre,atn PU,pu
        #ATN name1,val1,name2,val2
        og1.ATNWrite('previousPosition', previousPosition.__str__(),'position',pu.__str__())
        og1.ATNWrite('position',pu.__str__(),'previousPosition', previousPosition.__str__())
        

        #relate n1,v1,rel,relV,na2,v2
        og1.RELATEWrite('','POSITION',pu.__str__(),'relationship',"CS", "masterPickip", masterPU.__str__())

        #position, pu,atn,pu,pu
        og1.ATNWrite('POSITION', pu.__str__(),'previousPosition',previousPosition.__str__())
        # position pu atn DeN DeN
        og1.ATNWrite('POSITION', pu.__str__(),'DeN',DeN)
        # atn DeN DeN position pu
        og1.ATNWrite('DeN',DeN, 'POSITION',pu.__str__())
        #store rdf
        r0 = m[0]
...
def ctl4(): # </tag>
    global r0,parent,parentPosition,sequenceMode,chooseMode,pu,previousPosition,og1
    og1.ATNWrite("ENDPOINT",pu.__str__(),'','')
...
def ctl5(): #<name{atr}|/>,>
    global r0,d60,DeN,parent,parentPosition,attr,r1,og1
    d60 = 0
    a1 = r0.find(' ')
    if (a1 <> -1):
        d60 = d60 +1
...
#
def ctl100():
    global DeN,tnx,sequenceMode,chooseMode,masterPKI,masterPU,seq,og1
    #scrub xmlns
    s100 = DeN.find(':')
    if (s100 <> -1):
        tnx = DeN[s100+1:]
    ...
    if (tnx.upper() == 'ELEMENT'):
        ...
        if (sequenceMode==1):
            ...
            seq = seq+1
            attr['seq'] = seq
            og1.RELATEWrite('','POSITION',pu.__str__(),'relationship',"CS",'masterPickup',masterPU.__str__())
            og1.ATNWrite('POSITION',pu.__str__(),"seq",seq.__str__())
            getAttr()
            spoAttr()
            ctl = 1000
        elif (chooseMode == 1):
            ...
            seq = seq+1
            attr['seq'] = seq
            og1.RELATEWrite('','POSITION',pu.__str__(),'relationship',"CCH",'masterPickup',masterPU.__str__())
            #phase2
            og1.ATNWrite('POSITION',pu.__str__(),"seq",seq.__str__())
            getAttr()
            spoAttr()
...
def ctl1000():
    global l2x,parent,parentPosition,og1
    if (l2x == '/>'):
...
#
def spoP():
    global DeN,attr,pu,kri,nax,r1,l2x,snowflakeRtns,parent,parentPosition,l1x,og1
    print('--spoP')
    # best kri 
    #add DeN to parents
    parentPlus(DeN)
    ...
    og1.ATNWrite("PARENTDeN",prepu,'childDeN',DeN)
    ######spoPlus(DeN,'parentDeN',prepu)
    og1.ATNWrite("DeN",DeN,'parentDeN',prepu)
    og1.ATNWrite("l1x",l1x,'position',pu.__str__())
...
#
def spoAttr():
    global DeN,attr,pu,kri,parent,parentKRID,s,DeNftsw,snowflakeRtns,og1,parentPosition,p2pix
    print('--spoAttr')
    kri = getKRID()
    #attributes spo
    ...
    ######spoPlus(DeN,'parentDeN',prepu)
    og1.ATNWrite('DeN',DeN,'parentDeN', parent[len(parent)-2].__str__() )
    print('\ndump of attr')
    print(attr)
    ...
    try:
        j = attr[ref] 
        og1.ATNWrite("POSITION" ,pu.__str__() , "ReferentPosition" ,masterPU.__str__())
        og1.ATNWrite('PHASE2',p2pix.__str__(), "REFProcessPosition", pu.__str__())
        p2pix = p2pix + 1 
    except:
        nop = 1
    finally:
    ...
    if (DeNftsw == 0):
        ...
        og1.ATNWrite('FIRST',kri,'','')
        DeNftsw = 1
    #endif
    # add KRID to atn profile
    og1.ATNWrite("KRID",kri,"KRID",kri)
    ######spoPlus("ATN","("+ "FILE" + '=' + s.fh.name + ")",kri)
    prepu = parentPosition[len(parentPosition)-2]
    prepu = prepu.__str__()
    og1.ATNWrite("parentPosition", prepu, "KRID",kri)
    ###
    ###
    og1.ATNWrite("KRID",kri, "parentPosition", prepu)
    
    
    og1.ATNWrite("FILE",s.fh.name, "KRID",kri)
    ####
    ######spoPlus("ATV","("+ s.fh.name + '=' + "FILE" + ")",kri)
    ######spoPlus(kri,"ATN","("+ "FILE" + '=' + s.fh.name + ")")
    
    og1.ATNWrite("KRID",kri,"FILE",s.fh.name)
    ####
    ######spoPlus(kri,"ATV","("+ s.fh.name + '=' + "FILE" + ")")
    puat = getParentKRID()

    ######spoPlus(kri,'parentPickup',parentPosition[len(parentPosition)-2].__str__())
    og1.ATNWrite("KRID",kri,"parentKRID", puat)
    og1.ATNWrite("parentKRID",puat,"KRID",kri)
    og1.ATNWrite("parentKRID",puat,"SEQ",seq.__str__())
    og1.ATNWrite("parentKRID",puat,"KRID",kri)
    og1.ATNWrite("parentKRID",puat,"PEER",peer.__str__())
    ...
    for x in attr:
        ...
        if (ax == "position"):
            ...
            og1.ATNWrite( "POSITION", atx, "KRID",kri)
        #
        ...
        if (axs == "TYPE"):
            if (atxs == "STRING"):
                # skos kri , atn, ax,atx
                og1.ATNWrite("DEFKRID",kri,ax,atx)
                og1.ATNWrite(ax,atx,"DEFKRID",kri)
                # kri kri skosATN ax,atx
                og1.RELATEWrite('',"KRID",kri,"DEFATN",'',ax,atx)
                og1.RELATEWrite('',ax,atx,"DEFATN",'',"KRID",kri)
            #
        #
        ######spoPlus('IRI',ax,kri)
        ######spoPlus('IRI',atx,kri)
        ######spoPlus(kri,ax,atx)
        og1.ATNWrite("KRID",kri,ax,atx)
        ###
        og1.ATNWrite(ax,atx,"KRID",kri)
        ####
        ######spoPlus(kri,atx,ax)
...
# support 
def getParentchain():
    global krid,parent,parentPosition,og1
    # make parent chain in me:-:dad:-:Gpa
    parentchain = ""
    sx = len(parentPosition)-1
    for s1 in range(sx):
        jj = og1.ATNSeek('position',parentPosition[sx-s1].__str__(),'KRID')
        ## adjustments
        jj = jj[0][4].__str__()
...
def getKRID4PU(pu):
    global parentPosition,og1
    puat = ''
    try:
        puat = og1.ATNSeek('position',parentPosition[len(parentPosition)-2].__str__(),"KRID")
        puat = puat[0][4].__str__() # value2
    except:
        puat = ''
    finally:
        nop = 1
